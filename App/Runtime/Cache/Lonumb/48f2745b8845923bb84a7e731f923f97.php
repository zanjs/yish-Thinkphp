  ...
  <div class="main">
     ...
     <div class="m_list_operate">
       <dl>
         ...
         <!--dd><a href="/Lonumb/Admin/add">添加</a></dd-->
       </dl>
     ...
     <div class="m_list_table" id="listTable">
       <table cellspacing="0">
         ...
         <tbody>
         	<?php if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$userlist): $mod = ($i % 2 );++$i;?><tr>
             <td><input type="checkbox" name="<?php echo ($userlist["id"]); ?>"></td>
             <td><?php echo ($userlist["user"]); ?></td>  
             <td><?php echo ($userlist["regtime"]); ?></td> 
             <td><a href="/Lonumb/Admin/edit/id/<?php echo ($userlist["id"]); ?>">编辑</a><!--a onclick="return delList();" href="/Lonumb/Admin/del/id/<?php echo ($userlist["id"]); ?>">删除</a--></td>
...
function submitSplit(){
	spDd[0].onclick=function(){
    		location.href='/Lonumb/Admin/index'+sn_sub+np_sub+'/splitPage/1';
  }
    ...
    spDd[s].onclick=function(){
    		if(spDd[this.index].className=="m_list_page_on"){
    			...
    			location.href='/Lonumb/Admin/index'+sn_sub+np_sub+'/splitPage/'+spDd[this.index].innerHTML;
    		}
    }
	}		
	spDd[spDd.length-1].onclick=function(){
    		location.href='/Lonumb/Admin/index'+sn_sub+np_sub+'/splitPage/'+pageCount;
	}
}

function submitNum(){
	npBox.onchange=function(){
		location.href='/Lonumb/Admin/index'+sn_sub+'/numPage/'+npBox.value+sp_sub;
	}
}
function submitSearch(){
	snSearch.onclick=function(){
		if(snBox.value!=""){
			location.href='/Lonumb/Admin/index/searchName/'+snBox.value+np_sub+sp_sub;
		}else{
			location.href='/Lonumb/Admin/index'+np_sub+sp_sub;
		}
	}
}

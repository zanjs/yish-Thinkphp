  ...
  <div class="main">
     ...
     <div class="m_list_operate">
       <dl>
         ...
         <dd><a href="/Lonumb/Clients/add">添加</a></dd>
       </dl>
     ...
     <div class="m_list_table" id="listTable">
       <table cellspacing="0">
         ...
         <tbody>
             ...
             <td onmouseover="this.style.background='#000'" onmouseout="this.style.background=''"><img src="<?php echo ($lo['pic']); ?>" style="height:50px; display:block;"></td>
             <td><?php echo ($lo['porder']); ?></td>  
             <td><?php echo ($lo['status']); ?></td>  
             <td><?php echo ($lo['htime']); ?></td>
             <td>
             	<a href="/Lonumb/Clients/edit/id/<?php echo ($lo['id']); ?>">编辑</a>
             	<a onclick="return delList();" href="/Lonumb/Clients/del/id/<?php echo ($lo['id']); ?>">删除</a>
             </td>
...
spBox.onmouseover=function(){
	spDd[0].onclick=function(){
    		location.href='/Lonumb/Clients/index'+sc_sub+sn_sub+np_sub+'/splitPage/1';
  }
	for(var s=1;s<spDd.length;s++){
        ...
        spDd[s].onclick=function(){
    		if(spDd[this.index].className=="m_list_page_on"){
    			...
    			location.href='/Lonumb/Clients/index'+sc_sub+sn_sub+np_sub+'/splitPage/'+spDd[this.index].innerHTML;
    		}
        }
	}		
	spDd[spDd.length-1].onclick=function(){
    		location.href='/Lonumb/Clients/index'+sc_sub+sn_sub+np_sub+'/splitPage/'+pageCount;
	}
}

function submitNum(){
	npBox.onchange=function(){
		location.href='/Lonumb/Clients/index'+sc_sub+sn_sub+'/numPage/'+npBox.value; //+sp_sub
	}
}
function submitSearch(){
	snSearch.onclick=function(){
		if(snBox.value!=""){
			location.href='/Lonumb/Clients/index'+sc_sub+'/searchName/'+snBox.value.replace(/\s+/g,"not find")+np_sub; //+sp_sub
		}else{
			location.href='/Lonumb/Clients/index'+sc_sub+np_sub; //+sp_sub
		}
	}
}
...
scBox.onmouseover=function(){
	for(var l=0;l<=scA.length;l++){
        ...
        scA[l].onclick=function(){
    		return location.href='/Lonumb/Clients/index/subClass/'+scA[this.index].name+np_sub;
    	}
    }
}
